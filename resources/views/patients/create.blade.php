<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ isset($patient) ? 'Edit' : 'Tambah' }} Data Pasien
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ isset($patient) ? route('patients.update', $patient) : route('patients.store') }}" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <select id="hospital_id" name="hospital_id" required
                            ...
                            @foreach($hospitals as $hospital)
                                <option value="{{ $hospital->id }}" {{ (isset($patient) && $patient->hospital_id == $hospital->id) ? 'selected' : '' }}>
                                    {{ $hospital->nama }}
                                </option>
                            @endforeach
                        </select>
